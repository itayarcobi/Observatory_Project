	...
	class ClientTest
	{
		static void Main(string[] args)
		{
			...
			if (progID != "")
            {
			    ...
			    Console.WriteLine("  The Focuser current position is " + F.Position);
				Console.WriteLine("Enter the new position to which the focuser will move:");
				//int nfp = (int)(0.7 * F.Position);
				int nfp = Convert.ToInt32(Console.ReadLine());
				//int nfp = (int)(10101);
			    ...
			    Console.WriteLine("\r\n  Move complete.\n The new position is " + F.Position.ToString());
			    F.Link = false;
			...
			//string filepath = "C:\Users\Orkabi\Desktop\ClientToolkitSampleCSharp\json\j_rotator.json";
			//using (StreamReader r = new StreamReader(filepath))
			//{
			//	var json = r.ReadToEnd();
			//	var jobj = JObject.Parse(json);

			//	var arr = jobj.Properties().ToArray();
			//	float np = (arr[2].Value);
			//	Console.WriteLine(azimut);
			...
			if (progID != "")
			{
				...
				Console.WriteLine("  The Rotator current position is " + R.Position);
				Console.WriteLine("Enter the new position to which the rotator will move:");
				float np = Single.Parse(Console.ReadLine());
				if (np >= 360) np -= 360;
